...
Item {
    ...
    function update() {
        ...
        var cmd = pluginApi.pluginSettings.updateCmd || pluginApi.manifest.metadata.defaultSettings.updateCmd
        ...
        var dynamicExcludes = preserveList ? preserveList.split(/\s+/).map(function(name) { return '! -name ' + name }).join(" ") : ""
        dynamicExcludes += ' ! -name ksycoca6_*'
        cmd = cmd.replace(/EXCLUDE_DIRS="[^"]*"/, 'EXCLUDE_DIRS="' + dynamicExcludes.trim() + '"')
        cmd = cmd.replace(/find ~\/\.cache/g, 'set -f; find ~/.cache')
        runUpdate.command = ["sh", "-c", cmd]
