...
ColumnLayout {
    ...
    NTabView {
        ...
        ColumnLayout { // General Tab
            ...
            // Cleanup After Update Toggle
            Item {
                Layout.fillWidth: true
                Layout.preferredHeight: cleanupToggle.implicitHeight
                NToggle {
                    id: cleanupToggle
                    anchors.fill: parent
                    label: pluginApi.tr("settings.general.cleanupAfterUpdate.text")
                    description: pluginApi.tr("settings.general.cleanupAfterUpdate.desc")
                    checked: root.cleanupAfterUpdate
                    onToggled: checked => root.cleanupAfterUpdate = checked
                }
            }

            NTextInput { // Cleanup Command
                Layout.fillWidth: true
                label: pluginApi.tr("settings.general.cleanupCmd.text")
                description: pluginApi.tr("settings.general.cleanupCmd.desc")
                placeholderText: pluginApi.manifest.metadata.defaultSettings.cleanupCmd
                text: root.cleanupCmd
                onTextChanged: {
